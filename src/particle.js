...
import images from '../game/imageBuffer.js'

import { SCENE } from '../game/environment.js'
...
var Particles = function(obj) {
  ...
  ! function applyTexture(tex) {
    if (tex === undefined) {
      self._map = images.particleTexture
    } else {
        ...
        var interval = setInterval(() => {
          if (self._end) {
            ...
            if (self._p.size.isRandom) {
              ...
              if (self._p.mesh.type === "Sprite") {
                self._p.mesh.scale.x += Math.random() * (.5 - (-.8) + (-.8)) //Math.random() * (self._p.size.maxSize - self._p.size.minSize) + self._p.size.minSize
                self._p.mesh.scale.z += Math.random() * (.5 - (-.8) + (-.8)) // Math.random() * (self._p.size.maxSize - self._p.size.minSize) + self._p.size.minSize
            ...
            if (self._p.mesh === undefined) {
              ...
              if (self._p.mesh.type === "Mesh") {
                ...
                if (self._p.randomRotation) {
                  ...
                  particle.rotation.set(rot, 0, rot)

                } else {
                  particle.rotation.copy(self._p.mesh.rotation)
                }
              ...
              } else if (self._p.mesh.type === "Sprite") {
                particle = new three.Sprite(self._p.mesh.material)
              }
            }
